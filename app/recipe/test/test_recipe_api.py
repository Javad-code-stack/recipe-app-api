...
class PrivateRecipeApiTest(TestCase):
    ...
    def test_create_recipe_with_new_tags(self):
        ...
        1 - First we Create and post the payload
        2 - Check the status code it should be 201(Created)
        3 - Filter the recipes for authenticated users
        4 - Check the count of the recipes it's should be 1
                (Because it's a test, and we JUST created that one recipe here)
        5 - Assign that one recipe to a variable -- recipe = recipes[0]
            (This gives more info incase an error occurred)
        6 - BE CAREFUL we are counting the number of tags not recipes(#4)
        7 - Loop through each tag that expected to be created to be sure they
                exist we correct name and user
        """
        payload = {
            "title": "Peperoni-Salami pizza",
            "time_minutes": 20,
            "price": Decimal("5.99"),
            "tags": [{"name": "pizza"}, {"name": "dinner"}],
        }
    ...
    def test_create_recipe_with_existing_tags(self):
        """Test if a recipe with existing tags exists"""
        tag_italian = Tag.objects.create(user=self.user, name="Italian")
        payload = {
            "title": "Peperoni-Salami pizza",
            "time_minutes": 25,
            "price": Decimal("8.99"),
            "tags": [{"name": "Italian"}, {"name": "lunch"}],
        }
    ...
    def test_create_recipe_with_existing_ingredients(self):
        """Test creating existing ingredient"""
